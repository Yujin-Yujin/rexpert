...
class BertSelfOutputAdaptersMixin:
    ...
    def adapter_fusion(self, hidden_states, adapter_stack, residual, query):
        ...
        #     )
        # return hidden_states
        # pooh attention_prob added
            hidden_states, fusion_attention_probs = self.adapter_fusion_layer[fusion_name](query, up_list, up_list, residual)
        # pooh attention_prob added
        return (hidden_states, fusion_attention_probs)

    def adapters_forward(self, hidden_states, input_tensor, adapter_names=None):
        # pooh added fusion_attention_outputs
        fusion_attention_outputs = False
        ...
        if adapter_names is not None and (
            ...
            # pooh added fusion_attention_outputs
            fusion_attention_outputs = True
            for adapter_stack in adapter_names:
                # pooh added fusion_attention_probs
                hidden_states, fusion_attention_probs = self.adapter_stack_layer(
        ...
        return (hidden_states, fusion_attention_probs) if fusion_attention_outputs else (hidden_states,)
...
class BertOutputAdaptersMixin:
    ...
    def adapter_stack_layer(self, hidden_states, input_tensor, adapter_stack):
        ...
        pooh changed Returns : if len(adapter_stack) >1: Returns (hidden_states, fusion_attention_probs)
        """
        # We assume that all adapters have the same residual connection and layer norm setting as the first adapter in
        # the stack
        adapter_config = self.config.adapters.get(adapter_stack[0])
    ...
    def adapter_fusion(self, hidden_states, adapter_stack, residual, query):
        ...
        pooh changed Returns: hidden_states, fusion_attention_probs
    ...
    def adapters_forward(self, hidden_states, input_tensor, adapter_names=None):
        ...
        if adapter_names is not None and (
            ...
            for adapter_stack in adapter_names:
                ...
                outputs = self.adapter_stack_layer(
                    hidden_states=hidden_states,
                    input_tensor=input_tensor,
                    adapter_stack=adapter_stack,
                )
