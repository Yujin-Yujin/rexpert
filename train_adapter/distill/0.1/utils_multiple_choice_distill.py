    ...
    class MultipleChoiceDataset(Dataset):
        ...
        def __init__(
            ...
            cached_features_file = os.path.join(
                data_dir,
                "cached_{}_{}_{}_{}_{}_distill".format(
                    mode.value,
            ...
            with FileLock(lock_path):
                ...
                if os.path.exists(cached_features_file) and not overwrite_cache:
                    ...
                    if teacher_model is not None:
                        self.features = soft_label_features(
                            self.features,
                            teacher_model
                        )
...
def soft_label_features(
    example_features: List[InputFeatures],
    teacher_model
) -> List[InputSoftlabelFeatures]:
    ...
    soft_label_features = []
    for (ex_f_idx, example_feature) in tqdm.tqdm(enumerate(example_features), desc="convert features to softlabel features"):
        soft_label_features.append(
            InputSoftlabelFeatures(
                ...
                soft_label=logits[ex_f_idx]
            )
        )
        
    for f in soft_label_features[:2]:
        logger.info("*** Example ***")
        logger.info("soft_label_features: %s" % f)
    
    return soft_label_features
