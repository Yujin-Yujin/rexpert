...
class BertFusionPooler(nn.Module):
    def __init__(self, config):
        ...
        # self.choice_num = len(config.id2label.keys())
    ...
    def forward(self, hidden_states):
        ...
        # print("pooh BertFusionPooler hidden_states", pooled_output.shape)
        h_shape = pooled_output.shape
        batch_size=int(8)
        choice_num = int(h_shape[1] / batch_size)
        pooled_output = pooled_output.view(h_shape[0], -1, choice_num, self.dense_size).sum(2)

        # e = d.contiguous().view(h_shape[0], batch_size, -1)
        f = pooled_output.permute(1,0,2)
        new = []
        for i, x in enumerate(f):
            new.append(x)

        pooled_output = torch.stack(new).view(-1, self.dense_size)
...
class BertFusion(nn.Module):
    ...
    """
    # query = adapter's representations
    # key = classifier layer
    def __init__(self, config):
        ...
        if self.config.adapter_fusion["key"]:
            self.key = nn.Linear(self.dense_size, 1)
        ...
        self.pooler = BertFusionPooler(self.config)
