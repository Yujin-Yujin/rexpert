...
class ATOMICProcessor(object):
	def __init__(self, args):
		...
		self.define_rel = ['ObjectUse', 'AtLocation', 'MadeUpOf', 'HasProperty']
		self.precondition_rel = ["isAfter", "xReason", "xNeed", "xIntent"]
	...
	def create_dataset(self, data):
		...
		for sample in tqdm(data):
			for k, v in sample[1].items():
				if len(v) != 0 and k in self.define_rel:
					context = text_standardize(ftfy.fix_text(sample[0]))
					if '___' not in context:
