...
class RobertaForMultipleChoice(ModelWithHeadsAdaptersMixin, RobertaPreTrainedModel):
    ...
    def forward(
        ...
        print("pooh pooled output shape", pooled_output.shape)

        adapter_names_list = adapter_names.split(",")
        print("pooh adapter_names_list", adapter_names_list)

        # pooled_output = self.dropout(pooled_output)
        # logits = self.classifier(pooled_output)
        reshaped_logits = logits.view(-1, len(adapter_names_list))
        raise RuntimeError()
