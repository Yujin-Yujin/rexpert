...
class InputClassifierFeatures:
    ...
    soft_label: Optional[List[float]]
        ...
        def __init__(
            ...
            if sub_model_list is not None and adapter_names is not None:
                cached_classifier_features_file = os.path.join(
                    data_dir,
                    "cached_{}_{}_{}_{}_{}_adapter_classifier_soft_label".format(
                        mode.value,
                        tokenizer.__class__.__name__,
                        str(max_seq_length),
                        task,
...
class ATOMICProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {} train".format(data_dir))
        return self._create_examples(self._read_json(os.path.join(data_dir, "train_random.jsonl")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {} dev".format(data_dir))
        return self._create_examples(self._read_json(os.path.join(data_dir, "dev_random.jsonl")), "dev")
    ...
    def _create_examples(self, lines, type):
        ...
        for line in tqdm.tqdm(lines, desc="read atomic data"):
            data_raw = json.loads(line.strip("\n"))
            examples.append(
                InputExample(
...
class CommonsenseqaProcessor(DataProcessor):
    """Processor for the SocialIQA data set."""
    # not use
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {} train".format(data_dir))
    ...
    def get_test_examples(self, data_dir):
        ...
        #     "setting!"
        # )
        return self._create_examples(self._read_json(os.path.join(data_dir, "test_rand_split.jsonl")), "test")
    ...
    def _create_examples(self, lines, type):
        ...
        for line in tqdm.tqdm(lines, desc="read commonsenseqa data"):
            data_raw = json.loads(line.strip("\n"))
            qa = data_raw["question"]
            # print("1", data_raw["question"]['choices'][0]['text'][0].lower()+data_raw["question"]['choices'][0]['text'][1:])
            # print("2", data_raw["question"]['choices'][0]['text'])
            ...
            examples.append(
                ...
                contexts=["", "", "", "", ""],
                endings=[item['text'][0].lower()+item['text'][1:] for item in data_raw["question"]['choices']],
                label=data_raw['answerKey'],
                )
            )
...
class KGProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, type):
        ...
        for line in tqdm.tqdm(lines, desc="read multikg data"):
            data_raw = json.loads(line.strip("\n"))
            
            if "question" in data_raw.keys():
                qa = data_raw["question"]
                ...
                examples.append(
                    ...
                    contexts=["", "", "", "", ""],
                    endings=[item['text'][0].lower()+item['text'][1:] for item in data_raw["question"]['choices']],
                    label=label_map[data_raw['answerKey']],
                    )
                )
            else:
                examples.append(
...
def convert_examples_to_classifier_features(
    ...
    for i in range(len(sub_model_logits[0])):
        ...
        for j in range(len(sub_model_logits)):
            answer_probs.append(sub_model_logits[j][i][labels[i]])
        # answer_probs = torch.tensor(answer_probs)
        adapter_labels.append(answer_probs)

    # adapter_labels = torch.stack(adapter_labels)
    ...
    for (ex_f_idx, example_feature) in tqdm.tqdm(enumerate(example_features), desc="convert features to classifier features"):
        classifier_features.append(
            InputClassifierFeatures(
                ...
                label=labels[ex_f_idx],
                soft_label=adapter_labels[ex_f_idx]
            )
        )
