...
from utils_multiple_choice_soft import MultipleChoiceDataset, Split, processors
import pandas as pd
import wandb
import shutil
...
class LinearFusion(nn.Module):
    ...
    def forward(
        ...
        if soft_label is not None:
            ...
            soft_reshaped_logits = nn.Softmax(dim=-1)(reshaped_logits)

            # label = (label.unsqueeze(0)).transpose(1,0)
            # label = torch.repeat_interleave(label, repeats=3, dim=0) 
            # label = torch.repeat_interleave(label, repeats=fusion_attention_probs.shape[1], dim=1) 
            ...
            loss = loss_fct(soft_reshaped_logits, soft_label)
