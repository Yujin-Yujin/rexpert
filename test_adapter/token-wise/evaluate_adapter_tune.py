...
def main():
    ...
    parser.add_argument("--adapter_load", default=None, type=str, required=False, help="adapter path")
    parser.add_argument("--adapter_names", default=None, type=str, required=False, help="adapter names")
    parser.add_argument("--fusion_path", default=None, type=str, required=False, help="fusion path")
    ...
    if args.adapter_names is not None:
        out_file = os.path.join(out_dir, '{}-predictions.jsonl'.format(args.adapter_names))
        log_file = os.path.join(out_dir, '{}-results.txt'.format(args.adapter_names))
    else:
    ...
    model, tokenizer = init_model(args.lm, device, args.cache_dir, args.adapter_names, args.adapter_load, args.fusion_path)
    ...
    results = []
    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else 0
    print('using', args.adapter_names)
    print ('currently evaluating the task', task, args.dataset_file)
    # Predict instances
    sample_id = 0
...
def get_lm_score(model, batch, label_ids, attention_mask):
    ...
    with torch.no_grad():
        ...
        # print("before",batch.is_cuda)
        # batch = batch.to(device)
        # print("after", batch.is_cuda)

        # print("before",attention_mask.is_cuda)
        # attention_mask = attention_mask.to(device)
        # print("after", attention_mask.is_cuda)
...
def init_model(model_name: str,
               device: torch.device, cache_dir, adapter_names, adapter_load, fusion_path):
    """
    ...
    if adapter_names is not None:
        adapter_names_list = [adapter_names.split(',')]
        if len(adapter_names_list[0]) == 1:
            adapter_path = os.path.join(adapter_load,adapter_names)
            adapter = model.load_adapter(adapter_path)
            model.set_active_adapters(adapter)

        elif len(adapter_names_list[0]) >1 and fusion_path is not None:
            for adapter_name in adapter_names_list[0]:
                print("pooh adapter_name", adapter_name)
                adapter_path = os.path.join(adapter_load,adapter_name)
            ...
            model.set_active_adapters(adapter_names_list)
