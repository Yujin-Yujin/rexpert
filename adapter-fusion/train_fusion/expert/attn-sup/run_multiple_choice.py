...
from utils_multiple_choice_attn_sup import MultipleChoiceDataset, Split, processors
import pandas as pd
import wandb
...
class FusionArguments:
    ...
    pretrained_submodel_fusion_dir_path: Optional[str] = field(
        default=None, metadata={"help": "submodel_fusion_dir_path of pretrained adapters"}
    )
...
def main():
    ...
    wandb.init(project="emnlp-10k", name="attn-15-{}".format(fusion_args.pretrained_adapter_names))
    ...
    if fusion_args.test_fusion:
        ...
        print("fusion training is started!")
        trainer = Trainer(
            model=model,
            args=training_args,
