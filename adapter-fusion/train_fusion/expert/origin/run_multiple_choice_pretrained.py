...
def main():
    ...
    if (fusion_args.train_fusion + fusion_args.test_fusion) >0:
        ...
        model.load_adapter("comsense/siqa@ukp", load_as="siqa", config=config)
        model.load_adapter("comsense/csqa@ukp", load_as="csqa", config=config)

        if fusion_args.train_fusion:
            model.add_fusion(Fuse("siqa", "csqa"))
            model.set_active_adapters(Fuse("siqa", "csqa"))
            model.train_fusion(Fuse("siqa", "csqa"))
        else:
            fusion_path = fusion_args.fusion_path
            model.load_adapter_fusion(fusion_path)
            model.set_active_adapters(Fuse("siqa", "csqa")) # is this code needed? yes
