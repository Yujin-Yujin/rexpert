...
from utils_multiple_choice_distill import MultipleChoiceDataset, Split, processors
import pandas as pd
import wandb
import shutil
...
class DataTrainingArguments:
    ...
    teacher_model_path: Optional[str] = field(
        default=None, metadata={"help": "Teacher model path"}
    )
...
def main():
    ...
    if data_args.teacher_model_path is not None:
        config = AutoConfig.from_pretrained(
                    model_args.config_name if model_args.config_name else model_args.model_name_or_path,
                    num_labels=num_labels,
                    finetuning_task=data_args.task_name,
        ...
        adapter_path = os.path.join(data_args.teacher_model_path)
        adapter = teacher_model.load_adapter(adapter_path)
        teacher_model.set_active_adapters(adapter)
