...
class ATOMICMLMProcessor(object):
	def __init__(self, args):
		self.D = []
		self.filelist = [args.train_file, args.validation_file]
...
class ATOMICProcessor(object):
	def __init__(self, args):
		print ('loading from %s %s' % (args.train_file, args.validation_file))
		self.filelist = [args.train_file, args.validation_file]
		self.D = [[], []]
...
class CWWVProcessor(object):
	def __init__(self, args):
		...
		if args.task_name == 'cskg':
			print ('loading from %s %s' % (args.second_train_file, args.second_validation_file))
			self.filelist = [args.second_train_file, args.second_validation_file]
		else:
			print ('loading from %s %s' % (args.train_file, args.validation_file))
			self.filelist = [args.train_file, args.validation_file]
