...
def main():
    ...
    if data_args.train_file.endswith(".csv"):
        ...
        # df_trn = pd.read_json(data_files["train"], lines=True)
        # df_dev = pd.read_json(data_files["validation"], lines=True)
        # dataset = {"train": df_trn, "dev": df_dev}
    ...
    # Setup adapters
    if adapter_args.train_adapter:
        task_name = data_args.task_name or "cls"
        # check if adapter already exists, otherwise add it
        if task_name not in model.config.adapters:
            # resolve the adapter config
            adapter_config = AdapterConfig.load(
    ...
    # if training_args.do_predict is not None or data_args.test_file is not None:
