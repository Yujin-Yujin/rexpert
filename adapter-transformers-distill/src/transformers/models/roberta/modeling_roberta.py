...
class RobertaForMultipleChoice(ModelWithHeadsAdaptersMixin, RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.init_weights()
        print("distill only - softmax adapter training!")
        # print("CE + distill - softmax adapter training! ")
    ...
    def forward(
        ...
        if soft_label is not None:
            ...
            soft_soft_label = nn.Softmax(dim=-1)(soft_label)
            soft_reshaped_logits = nn.Softmax(dim=-1)(reshaped_logits)
            loss = (loss_kl(soft_reshaped_logits.log(), soft_soft_label))
